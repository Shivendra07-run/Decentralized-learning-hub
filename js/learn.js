/**
 * AETHER LEARN PAGE CONTROLLER (IIFE + window.Aether)
 * Handles:
 * 1. Pure-JS & Web Crypto SHA-256 hasher for Blockchain section
 * 2. Reading progress tracking (6 sections) synced with home page roadmap
 * 3. Desktop sticky Table of Contents & mobile chip bar with scroll-spy
 * 4. 3D background chain block highlighting
 */
...
(function () {
  ...
  /**
   * Pure-JS SHA-256 Implementation (Zero-dependency fallback)
   */
  ...
  function saveReadState(state) {
    try {
      ...
      // Synchronize with Home Page Roadmap checklist
      var roadmapState = {};
      try {
    ...
    function onScroll() {
      ...
      mobileChips.forEach(function (chip, idx) {
        if (idx === activeIdx) {
          ...
          // Smooth auto-scroll chip into view horizontally
